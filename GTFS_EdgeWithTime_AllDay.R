...
GTFS_EdgeWithTime <- function(dir_GTFS, dir_final, output = "shp"){
    ...
    for(trip in unlist(unique(stop_times[,.(trip_id)]) )){
      ...
      # in case of arrival / departure time formatted as: "24:MM:SS" (H >= 24)
      # - add +1 day and substract 24 hours (aday and dday, respectively)
      ...
      # calculate travel time from previous stop (travel_time, in minutes) & add stop ID of the previous stop (stop_idO)
      TempST[, c("stop_idO", "travel_time") := list(shift(stop_id), 
               round(as.numeric(times(chron(times = chron(times = arrival_time) + aday - 
                                        (chron(times = shift(departure_time)) + shift(dday)))))*60*24, 2) ) ]
      
      # add rows (except first) to edges
      edges <- rbind(edges, TempST[stop_sequence != 0,..Names_edges])
      rm(TempST, trip)
    }
    ...
    # merge orgins to gets their coordinates: destination
